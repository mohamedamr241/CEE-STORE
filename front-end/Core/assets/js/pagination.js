document.querySelector('#nav').addEventListener('click', function (evt) {
    if (evt.target.nodeName === 'LI') {
        ...
        addRedColor(evt.target)
        postData('/product/all',{num:num})
        .then(function(Data){
            updateUI(Data.data);
        })
        .then(function(){
    ...
    icons.forEach(icon => {
        icon.addEventListener("click", async(e) => {
            e.preventDefault();
            await addToCartt((icon.nextElementSibling).nextElementSibling, (icon.nextElementSibling));
...
const addRedColor = (target)=>{
    try{
        const listOfLi = document.querySelectorAll('.liii');
...
const addToCarttt = async(elem,Atag)=>{
    try{
        ...
        postData('/product/getProductId',{img:imageName})
        .then(async function(Data){
            if(Data.status=='success'){
                const result = await postData('/cart/addToCart',{productid:Data.data.result,quantity:1});
            }
        })
    }
    catch(err){
...
const updateUI = (data)=>{
    try{
        ...
        const arrayOfObjects = Object.values(data).map(obj => obj);
        ...
        console.log(`error in inserting products to the UI: ${err}`);
    }
}