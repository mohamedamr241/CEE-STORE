...
const addToCartt = async(elem,Atag)=>{
    try{
        ...
        postDataa('/product/getProductId',{img:imageName})
        .then(async function(Data){
            if(Data.status=='success'){
                const result = await postDataa('/cart/addToCart',{productid:Data.data.result,quantity:1});
            }
        })
    }
    catch(err){
